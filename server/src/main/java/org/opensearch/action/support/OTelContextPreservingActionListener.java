...
/**
 * Preservers the context and restores the given {@link ThreadContext.StoredContext}
...
public final class OTelContextPreservingActionListener<R> implements ActionListener<R> {
    ...
    private final long startCPUUsage;
    private final long startMemoryUsage;
    private final long startThreadContentionTime;
    ...
    public void onResponse(R r) {
        try (Scope ignored = Objects.requireNonNull(afterAttachContext).makeCurrent()) {
            ...
            if (spanID != null && Span.current() != Span.getInvalid() &&
                ...
                span.setAttribute(AttributeKey.longKey(OtelService.CPU_USAGE),
                    OtelService.getCPUUsage(Thread.currentThread().getId())- startCPUUsage);
                span.setAttribute(AttributeKey.longKey(OtelService.MEMORY_USAGE),
                    OtelService.getMemoryUsage(Thread.currentThread().getId())- startMemoryUsage);
                span.setAttribute(AttributeKey.longKey(OtelService.CONTENTION_TIME),
    ...
    public Context getAfterAttachContext() {
        return afterAttachContext;
    }

    public long getStartCPUUsage() {
    ...
    public long getStartThreadContentionTime() {
        return startThreadContentionTime;
    }
}
